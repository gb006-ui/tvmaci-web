window.addEventListener("DOMContentLoaded", () => {
    ...
    // ===== UGRÁLÓS JÁTÉK NYITÁS/ZÁRÁS – ÁTMENETTEL =====
    ...
    function handlePuzzleClick(e) {
        ...
        // Skálázás, hogy mobilon ne csússzon el
    ...
    // PUZZLE MEGNYITÁS / BEZÁRÁS – ÁTMENETTEL
    ...
    // MEMÓRIA MEGNYITÁS / BEZÁRÁS – ÁTMENETTEL
    ...
    const pongLeftBtn = document.getElementById("pongLeftBtn");
    const pongRightBtn = document.getElementById("pongRightBtn");
    ...
    function startPong() {
        ...
        let p1 = { x: 40, y: 155, w: pWidth, h: pHeight, dy: 0, score: 0 }; // Paprikajancsi (bal)
        let p2 = { x: pongCanvas.width - 40 - pWidth, y: 155, w: pWidth, h: pHeight, dy: 0, score: 0 }; // TV Maci (jobb)

        let ball = { x: pongCanvas.width / 2, y: pongCanvas.height / 2, r: 10, dx: 3.5, dy: 2.2 };
        ...
        // MOBIL IRÁNYÍTÓGOMBOK PONGHOZ
        let p1TouchActive = false;
        let p2TouchActive = false;

        function handleP1TouchStart(e) {
            e.preventDefault();
            if (!pongRunning) return;
            p1TouchActive = true;
            p1.dy = -5;
        ...
        function handleP1TouchEnd(e) {
            e.preventDefault();
            if (!pongRunning) return;
            p1TouchActive = false;
            p1.dy = 5;
            setTimeout(() => {
                if (!p1TouchActive) p1.dy = 0;
            }, 150);
        }

        function handleP2TouchStart(e) {
            e.preventDefault();
            if (!pongRunning) return;
            p2TouchActive = true;
            p2.dy = -5;
        ...
        function handleP2TouchEnd(e) {
            e.preventDefault();
            if (!pongRunning) return;
            p2TouchActive = false;
            p2.dy = 5;
            setTimeout(() => {
                if (!p2TouchActive) p2.dy = 0;
            }, 150);
        }

        if (pongLeftBtn && pongRightBtn) {
            pongLeftBtn.addEventListener("touchstart", handleP1TouchStart, { passive: false });
            pongLeftBtn.addEventListener("touchend", handleP1TouchEnd, { passive: false });

            pongRightBtn.addEventListener("touchstart", handleP2TouchStart, { passive: false });
            pongRightBtn.addEventListener("touchend", handleP2TouchEnd, { passive: false });

            // cleanup-be is betesszük
            pongState.cleanup = () => {
                document.removeEventListener("keydown", pongKeyDown);
                document.removeEventListener("keyup", pongKeyUp);

                pongLeftBtn.removeEventListener("touchstart", handleP1TouchStart);
                pongLeftBtn.removeEventListener("touchend", handleP1TouchEnd);
                pongRightBtn.removeEventListener("touchstart", handleP2TouchStart);
                pongRightBtn.removeEventListener("touchend", handleP2TouchEnd);
            };
        } else {
            pongState.cleanup = () => {
                document.removeEventListener("keydown", pongKeyDown);
    ...
    // PONG MEGNYITÁS / BEZÁRÁS – ÁTMENETTEL
    ...
    /* ============================
       MOBIL UGRÁS GOMB – SPACE SZIMULÁCIÓ
    ============================ */
